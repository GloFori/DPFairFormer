...
def main():
    ...
    with torch.no_grad():
        ...
        perf = evaluate_link_sign(model, z, data.test_pos_edge_index, data.test_neg_edge_index)
        fair = delta_dpsp(model, z, data.test_pos_edge_index, data.test_neg_edge_index, data.head_mask)

    metrics = {**perf, **fair}
    print(
        "Test AUC={auc:.4f} F1_POS={f1_pos:.4f} F1_NEG={f1_neg:.4f} "
